...
// âœ… LOAD TV SERIES EPISODES (using original styling)
async function loadTVSeriesData(tmdbId, imdbId, title) {
    try {
        ...
        for (let seasonNum = 1; seasonNum <= Math.min(seasons, 5); seasonNum++) {
            ...
            webSeriesData.innerHTML += `<h3>Season ${seasonNum}:</h3>`;
            
            const episodeContainer = document.createElement('div');
            episodeContainer.classList.add('episode-container'); // Using original CSS class
            
            let episodeLinks = '';
            seasonData.episodes.forEach(ep => {
                const episodeNumber = ep.episode_number.toString().padStart(2, '0');
                episodeLinks += `<a href="#" class="episodes" cssIdentification="s${seasonNum}e${ep.episode_number}" onclick="playEpisode(${tmdbId}, '${imdbId}', '${title}', ${seasonNum}, ${ep.episode_number}); return false;">E${episodeNumber}. ${ep.name}</a>`;
            });
        ...
        episodeContainers.forEach(container => container.classList.add('visible'));
...
window.playEpisode = function(tmdbId, imdbId, title, season, episode) {
    ...
    document.querySelector(`[cssIdentification="s${season}e${episode}"]`).classList.add('selected');
...
console.log("ðŸŽ¬ MovieFix with real-time search loaded successfully!");